...
export default {
  ...
  props: {
    ...
    obj: { type: Object },
    receiveShadow: {type: Boolean, default: true}, //接收阴影
    castShadow: {type: Boolean, default: true}, //投射阴影
    scale: { type: [Object, Number] }, //缩放 { x, y, z }
    position: { type: Object }, // 位置{ x, y, z }
    rotation: { type: Object } // 旋转{ x, y, z }
  },
  ...
  watch: {
    receiveShadow: {
      deep: true,
      handler(v) {
        this.curObj.receiveShadow = v
      }
    },
    castShadow: {
      deep: true,
      handler(v) {
        this.curObj.castShadow = v
      }
    },
    scale: {
      deep: true,
