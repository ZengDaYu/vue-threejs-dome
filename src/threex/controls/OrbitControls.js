...
export default function OrbitControls (object, domElement, global) {
  ...
  this.saveState = function () {
    ...
    scope.zoom0 = global.camera.zoom
  }
  ...
  this.reset = function () {
    ...
    global.camera.zoom = scope.zoom0
  ...
  let pan = function () {
    ...
    return function pan(deltaX, deltaY) {
      ...
      if(global.camera instanceof THREE.PerspectiveCamera) {
        // perspective
        let {position} = scope.object
        offset.copy(position).sub(scope.target)
        let targetDistance = offset.length()
        // 视场的一半在屏幕的中心到顶部
        targetDistance *= Math.tan((global.camera.fov / 2) * Math.PI / 180.0)

        // 实际上不使用屏幕宽度，因为透视相机是固定在屏幕上的
        panLeft(2 * deltaX * targetDistance / element.clientHeight, scope.object.matrix)
        panUp(2 * deltaY * targetDistance / element.clientHeight, scope.object.matrix)

      } else if(global.camera instanceof THREE.OrthographicCamera) {
        // orthographic,直角？
        panLeft(deltaX * (scope.object.right - scope.object.left) / global.camera.zoom / element.clientWidth, scope.object.matrix)
        panUp(deltaY * (scope.object.top - scope.object.bottom) / global.camera.zoom / element.clientHeight, scope.object.matrix)
  ...
  function dollyOut(dollyScale) {
    ...
    if(global.camera instanceof THREE.PerspectiveCamera) {
      ...
      global.camera.zoom = Math.max(scope.minZoom, Math.min(scope.maxZoom, global.camera.zoom * dollyScale))
      global.camera.updateProjectionMatrix()
      zoomChanged = true
  ...
  this.handleMove = function () {
    if(moveEvent) {
      ...
      if(this.stroked) {
        const intersects = getIntersects(moveEvent)
        if(intersects.length > 0) {
          const seleted = intersects[0].object
          selectedObjects = []
          selectedObjects.push(seleted)
          if(outlinePass) {
  ...
  // 添加沟边效果
  this.useStroke = function(renderer) {
    if(!global.scene || !global.camera) {
      return
    }
    ...
    renderPass = new RenderPass(global.scene, global.camera)
    this.composer.addPass(renderPass)

    outlinePass = new OutlinePass(new THREE.Vector2(window.innerWidth, window.innerHeight), global.scene, global.camera)
    this.composer.addPass(outlinePass)
    const loader = new THREE.TextureLoader()
    loader.load('static/textures/tri_pattern.jpg', function (texture) {
